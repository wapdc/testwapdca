...
class LinkitSearchPluginEntity extends LinkitSearchPlugin {
  ...
  function createPath($entity) {
    ...
    // Handle uuid based sites.
    if (module_exists('uuid') && $this->profile->data['insert_plugin']['url_method'] == LINKIT_URL_METHOD_UUID) {
      if (isset($entity->uuid)) {
        $path = '[uuid-link:' . $this->plugin['entity_type'] . ':' . $entity->uuid . ']';
        return $path;
      }
    }
  ...
  public function fetchResults($search_string) {
    ...
    if (module_exists('title')) {
      ...
      if ($title_in_use) {
        ...
        //$this->query->addMetaData('linkit_base_entity_', $this->plugin['entity_type']);
